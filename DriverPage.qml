...
Item {
    ...
    // TEMPORARY PRESENTATION SCALER (Innovation Forum demo on 27" screen).
    // The dashboard is designed pixel-perfect for an 800x480 driver display.
    // Wrap the whole UI in a fixed-size stage and uniformly scale it up to
    // fill the actual window. Revert by removing this wrapper.
    Item {
        id: stage
        width: 800
        height: 480
                    ...
                    ColumnLayout {
                        ...
                        RowLayout {
                            ...
                            Image {
                                ...
                                property bool isCritical: root.telemetry ? (root.telemetry.voltage <= 500.0) : false//--------------------
                        ...
                        ShapePath {
                            ...
                            PathAngleArc {
                                ...
                                sweepAngle: {
                                    ...
                                    // Scale matches the 0-120 km/h tick labels around the arc.
                                    var val = heroGaugeContainer.animatedSpeed;
                                    var ratio = Math.max(0.001, Math.min(val / 120.0, 1.0));
                                    return 162 * ratio;
                                }
                            }
                        }
